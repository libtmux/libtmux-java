      ...
      Server
        ...
        .use { server =>
          for {
            ...
            captured <- pane.awaitText("cats-owned-marker", deadlineFD)
            _ <- IO(
              assert(
    ...
    OwnedTmux.use { fixture =>
      ...
      Server
        ...
        .use { server =>
          for {
            operation <- server
              .cmd(Vector("run-shell", script), deadlineFD)
              .start
