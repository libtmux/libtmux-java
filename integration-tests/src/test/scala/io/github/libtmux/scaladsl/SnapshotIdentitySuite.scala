...
final class SnapshotIdentitySuite extends FunSuite {
  test("pure traversal preserves link occurrences and Java handle identity") {
    OwnedTmux.use { fixture =>
      ...
      // Server.fromJava is opaque, so closing a borrowed Server closes the
      // client it borrowed too; the Cats facade's fromJava leaves it open. A
      // closed client's isAlive throws rather than answering false, since that
      // asks about reachability, not this client's own state.
