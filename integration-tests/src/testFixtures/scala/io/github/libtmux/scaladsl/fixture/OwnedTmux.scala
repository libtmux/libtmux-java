...
private[scaladsl] object OwnedTmux {
  ...
  /** The tmux the Java fixture would use: `libtmux.tmux`, which each matrix
    * lane sets, or `tmux` from `PATH`, resolved to the executable it names.
    */
  def binary: String = {
    val named = sys.props.getOrElse("libtmux.tmux", "tmux")
    val path =
      if (named.contains('/')) Path.of(named)
      else
        sys.env
          .getOrElse("PATH", "")
          .split(java.io.File.pathSeparator)
          .iterator
          .map(Path.of(_).resolve(named))
          .find(Files.isExecutable(_))
          .getOrElse(
            throw new IllegalStateException(s"no executable $named on PATH")
          )
    require(
      path.isAbsolute && Files.isExecutable(path),
      s"libtmux.tmux must name an absolute executable: $path"
    )
    path.toString
  }
  ...
  // The build names the JDK a lane runs its suites on; a fixture started on any other has not
  // tested what the lane claims.
  private def assertForkedJdk(): Unit = {
    val selected = sys.props.getOrElse(
      "libtmux.testJdk",
      throw new IllegalStateException("libtmux.testJdk must name the test JDK")
    )
    assert(
      Runtime.version().feature() == selected.toInt,
      "test JVM is " + Runtime.version() + ", not the selected JDK " + selected
    )
  }
