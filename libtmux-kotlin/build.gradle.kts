...
// :libtmux's own Doclet output — the real operation-catalog.json (operation-catalog-schema.md),
// generated by :catalog-doclet walking :libtmux's @Operation methods. Read straight from :libtmux's
// build directory rather than its jar, so this task never unpacks an archive just to read one file.
val operationCatalog = project(":libtmux").layout.buildDirectory.file("generated/operation-catalog/operation-catalog.json")

// The checked-in TSV :libtmux's own field-catalog plugin also generates Pane_/Session_/Window_/
// Client_ from — read directly, so the two can never disagree.
val fieldCatalog = project(":libtmux")
    .layout.projectDirectory.file("src/main/resources/META-INF/io.github.libtmux/field-catalog.tsv")
...
val generateOperationWrappers =
    tasks.register<JavaExec>("generateOperationWrappers") {
        ...
        inputs.file(fieldCatalog).withPathSensitivity(PathSensitivity.RELATIVE)
        outputs.dir(output)
        ...
        argumentProviders.add(
            CommandLineArgumentProvider {
                listOf(
                    "--catalog",
                    operationCatalog.get().asFile.path,
                    "--outputDir",
                    output.get().asFile.path,
                    "--fields",
                    fieldCatalog.asFile.path,
                )
            },
        )
