...
- **3.2a ignores `new-window -c <dir>`** and `new-session -x/-y` (sizing),
  both silently, exit `0` either way. `split-window -c` *does* work on 3.2a,
  so the same flag is honored by one command and dropped by another with no
  error distinguishing the two. Both are refused before dispatch rather than
