...
shadowed by a Java member with only a compiler warning — proven by a compiled
probe, not assumed — so a member on a real wrapper class is what makes a
generated suspend mirror safe to add later without an accidental collision.
...
An earlier revision built the same bridge on `callbackFlow`, with the
readiness callback draining `poll()` straight into the flow's channel. Run
against a bursty producer racing a slow collector (`FlowBridgeStressTest`,
300,000 events), that version lost 240,892 of them with zero recorded as a
`Delivery.Gap`: a full channel's `trySend` failing discarded the polled item
silently. The `flow {}` version accounts for all 300,000, because nothing but
the collector itself ever calls `poll()` — an overflow can only ever happen
