...
These BOM-managed artifacts are built against each other. Mixing two releases
of them is the failure a platform exists to prevent, and it is easy to do by
hand when three coordinates each carry their own version string.
...
```text
runtimeClasspath
+--- io.github.libtmux:libtmux-bom:0.0.1-alpha.13
|    \--- io.github.libtmux:libtmux:0.0.1-alpha.13 (c)
\--- io.github.libtmux:libtmux -> 0.0.1-alpha.13
```

The `-> 0.0.1-alpha.13` on a coordinate you wrote without a version is the platform
doing its job.
