...
final class Typing {
    ...
    static Sent sendKeys(
        ...
        try (PaneInputReservations.Lease lease = stopping
                ? PaneInputReservations.interrupting(pane, cohort, "send_keys")
                : PaneInputReservations.keys(pane, cohort, "send_keys")) {
            if (!lease.passesThrough()) {
                return deliver(pane, keys, literal, enter, cohort, lease);
            }
            // The run already holds the shared pane lease. Enter it so sendKeys can stop the
            // command without taking that hold away from the run.
            try (PaneInput.Lease pass = PaneInput.enterInterrupt(pane)) {
    ...
    private static Sent deliver(
            Pane pane,
            List<String> keys,
            boolean literal,
            boolean enter,
            PaneInputCohort.Resolution cohort,
            PaneInputReservations.Lease lease) {
    ...
    static Pasted pasteText(Call call) {
        ...
        try (PaneInputReservations.Lease lease = PaneInputReservations.paste(pane, initial, "paste_text")) {
