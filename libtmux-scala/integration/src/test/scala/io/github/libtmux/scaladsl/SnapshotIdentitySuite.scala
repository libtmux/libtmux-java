...
import io.github.libtmux.exception.{ServerClosedException, TargetGoneException}
...
/** Opaque handles are the Java object itself, so identity here is exactly
  * Java's own `equals`/`hashCode` — physical identity for `Pane` (its
  * docstring: "Java equality identifies the physical pane"), not a hand-rolled
  * Scala comparison a wrapper could get wrong.
  */
final class SnapshotIdentitySuite extends FunSuite {
  test("pure traversal preserves link occurrences and Java handle identity") {
    OwnedTmux.use { fixture =>
      ...
      panes.foreach { pane =>
        ...
        assertEquals(pane, Pane.wrap(pane.asJava))
      ...
      // Server.fromJava is an opaque identity, not a separate wrapper the old, non-opaque facade
      // could make close a no-op for: closing a borrowed direct-style Server closes the client it
      // borrowed too (documented on Server.fromJava itself), unlike the Cats facade's own
      // fromJava, whose Resource release step deliberately leaves the borrowed client open. A
      // closed client cannot even ask whether the daemon is alive: isAlive throws rather than
      // answering false, since that is a question about reachability, not this client's own state.
      intercept[ServerClosedException](java.isAlive())
    }
  }
  ...
  test(
    ...
    OwnedTmux.use { fixture =>
      ...
      OwnedTmux.use { other =>
        val otherPane = Pane.wrap(other.server.panes().get(0))
      ...
      intercept[TargetGoneException](
        before.sendLiteral(Vector("must not reach replacement"))
      )
