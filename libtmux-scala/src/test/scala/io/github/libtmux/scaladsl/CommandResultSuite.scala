...
/** `transport.CommandResult` and `PaneRun` are Java's own records, used
  * directly: the deleted `CommandResult`/`PaneRun` wrappers only renamed their
  * accessors and converted `OptionalInt`/`List` at the boundary, which this
  * pins against the real types instead.
  */
