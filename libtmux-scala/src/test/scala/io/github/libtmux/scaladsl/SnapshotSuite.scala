...
/** `ServerSnapshot`/`PaneState`/... are Java's own records, used directly: the
  * deleted `Snapshot`/`PaneInfo` wrappers added nothing `ServerSnapshot` did
  * not already have (`windowsOf`, `panesOf`, `session`), so this pins the same
  * grouping and `Optional`/`OptionalLong` boundary behavior against the real
  * Java types instead.
  */
