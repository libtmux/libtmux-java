...
/** A live view of one tmux server's sessions, windows, panes and clients,
  ...
  * control subscription (which the first-round direct-style sketch did, leaking
  * one subscription per snapshot pulled). `close()` comes from the
  * `AutoCloseable` bound with no forwarding of its own.
  */
