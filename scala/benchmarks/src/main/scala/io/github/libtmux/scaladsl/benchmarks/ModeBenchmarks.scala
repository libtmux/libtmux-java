...
object ModeBenchmarks {
  ...
  private def scalaSourceSha256(): String = {
    val root = Iterator
      ...
      .takeWhile(_ != null)
      .find(path => Files.isDirectory(path.resolve("scala")))
      .getOrElse(
        throw new IllegalStateException("Scala repository root is missing")
      )
      .resolve("scala")
    val entries = Using.resource(Files.walk(root)) { paths =>
      paths
        .iterator()
        .asScala
        .filter(path =>
        ...
        .map { path =>
          root.relativize(path).toString.replace('\\', '/') + "\t" + sha256(
