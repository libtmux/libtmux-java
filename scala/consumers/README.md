...
```console
$ python3 scala/scripts/verify-consumers.py \
    --scala-stage scala/target/staging \
    --java-stage scala/target/java-repository \
    --version 0.0.1-alpha.12-scala-dev.1 \
    --java-version 0.0.1-alpha.12-scala-dev.1 \
    --docs-exports scala/examples/target/scala-2.13/resource_managed/test \
    --scala-version 2.13.18 \
    --jdk "$JAVA_HOME" \
    --tmux "$TMUX_TEST_BINARY" \
    --output scala/target/consumers/linux-jdk21-scala2.13.18
```
...
```console
$ python3 scala/consumers/test_inventory.py
```
