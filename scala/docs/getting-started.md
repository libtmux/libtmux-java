...
absolute path of the tmux binary used by live tests. The checked `scala/sbtw`
...
$ LIBTMUX_JAVA_VERSION=0.0.1-alpha.12-scala-dev.1 \
    ./scala/scripts/stage-java.sh
```

The default repository is `scala/target/java-repository`. The development
...
```console
$ LIBTMUX_JAVA_VERSION=0.0.1-alpha.12-scala-dev.1 \
    ./scala/sbtw crossUnit crossLive
```
...
The default Scala repository is `scala/target/staging`. Staging writes local
...
The generated Scaladoc starts at `scala/core/target/scala-2.13/api/index.html`
and `scala/cats/target/scala-2.13/api/index.html`. Scala 3 uses the
