...
def prepare(directory, tool, exports):
    template = ROOT / "scala/consumers"
    shutil.copytree(template / tool, directory, dirs_exist_ok=True)
    source_hashes = {}
    fixture = ROOT / (
        "scala/integration/src/test/scala/io/github/libtmux/scaladsl/fixture/OwnedTmux.scala"
    )
    ...
    if tool == "sbt":
        ...
        shutil.copyfile(ROOT / "scala/project/build.properties",
...
def runtime_inventory(path, kind, suffix, args):
    ...
    for line in path.read_text().splitlines():
        ...
        if group == GROUP:
            ...
            if kind != "direct" and sha != digest(artifact(stage, module, pinned)):
...
def commands(tool, directory, args):
    ...
    version = (ROOT / "scala/project/build.properties").read_text().strip().split("=")[1]
    ...
    if digest(launcher) != (ROOT / "scala/project/sbt-launch.sha256").read_text().strip():
...
def verify(args, staged):
    ...
    if args.output == ROOT or args.output == ROOT / "scala":
    ...
    try:
        for tool in ("sbt", "gradle") if args.tool == "both" else (args.tool,):
            ...
            if tool == "sbt":
                expected_tool = (ROOT / "scala/project/build.properties").read_text().strip().split("=")[1]
            else:
