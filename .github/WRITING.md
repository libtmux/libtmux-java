...
[`MIGRATION`](../MIGRATION) file. Cover renames, removals, and changes to
...
The README and getting-started guide show the current API. Link to `MIGRATION`
with a short upgrading sentence instead of embedding migration sections there.
Keep the changelog entry brief: state the impact and replacement, then link to
the migration notes for detailed steps.
